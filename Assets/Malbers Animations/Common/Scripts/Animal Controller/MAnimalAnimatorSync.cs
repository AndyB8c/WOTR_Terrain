...
namespace MalbersAnimations
{
    ...
    public class MAnimalAnimatorSync : MonoBehaviour
    {
        ...
        private void Awake()
        {
            ...
            if (animator != null)
            {
                listenTo = animator.GetComponent<IMAnimator>();
                StateCycle = animator.GetComponent<IAnimatorStateCycle>();
            }
        ...
        {
            if (animator.IsInTransition(0) || MyAnimator.IsInTransition(Layer)) return; //Do not Resync when is on  transition

            if (HasStateCheck(currentState))
            {
                var MasterStateInfo = animator.GetCurrentAnimatorStateInfo(0);
