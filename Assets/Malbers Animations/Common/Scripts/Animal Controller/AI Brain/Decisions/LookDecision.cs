    ...
    public class LookDecision : MAIDecision
    {
        ...
        private bool Look_For(MAnimalBrain brain)
        {
            switch (lookFor)
            {
                ...
                case LookFor.CurrentTarget: return LookForTarget(brain);
                case LookFor.TransformVar: return LookForTransformVar(brain);
                case LookFor.GameObjectVar: return LookForGoVar(brain);
        ...
        public bool LookForTransformVar(MAnimalBrain brain)
        {
            if (transform == null && transform.Value != null) return false;
            return IsInFieldOfView(brain, transform.Value.position, transform.Value);
        }

        public bool LookForGoVar(MAnimalBrain brain)
        {
            if (gameObject == null && gameObject.Value && !gameObject.Value.IsPrefab()) return false;
            return IsInFieldOfView(brain, gameObject.Value.transform.position, gameObject.Value.transform);
        }
